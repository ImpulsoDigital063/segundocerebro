# MPN-On — Status e Processamento

> Revisado no update geral de 04/07/2026 — DORMENTE desde ~16/04/2026: log Meta Ads parou em 16/04 (só 4 dias com gasto real, total R$140,72, zero purchase registrado); STATUS congelado em 01/05; nenhuma atividade nova.

**Atualizado:** 01 de Maio de 2026 (refresh — sem mudanças significativas na semana 28/04→01/05; foco do Eduardo foi AgendaPRO + Aura Energy)

---

## Situação atual

- Preço: R$297
- Campanhas: Ativas no Meta Ads
- Conversão: **Abaixo do esperado** — atenção pendente a VSL + Copy na próxima sprint
- Módulos gravados: Módulo 2 (29 aulas, 8+ horas)
- Módulos pendentes: 1, 3 e 4

> **Nota 01/05/2026:** MPN-On não recebeu atualizações na semana 28/04→01/05. Status desde 09/04 mantém-se. Próxima sprint MPN deve atacar VSL/copy pra destravar conversão antes de aumentar investimento Meta.

## Dados a coletar (urgente)

- [ ] CTR atual (Meta Ads)
- [ ] CPC atual
- [ ] Taxa de conclusão da VSL
- [ ] Conversão final (visitante → venda)
- [ ] Objeção #1 do público

## Testes em andamento

| Teste | Status | Resultado |
|---|---|---|
| VSL 10 min vs 23 min | Pendente | — |
| Copy objeção | Pendente | — |

## Histórico de ajustes

| Data | O que mudou | Resultado |
|---|---|---|
| — | — | — |

---

**Ver também:** [[EDUARDO-BARROS]] | [[PADROES-VALIDADOS]] | [[ESTRATEGIAS-ATIVAS]]

**Diagnóstico completo:** [[DIAGNOSTICO-MPN-ON-CONVERSAO]]  
**Roteiros:** [[ROTEIROS-MODULOS-1-3-4]]

**Status correlatos:** [[STATUS-IMPULSO]] · [[STATUS-AGENDAPRO]] · [[STATUS-RADARPRO]]
**Hubs:** [[MEGA-CLAUDE]] · [[EDUARDO-BARROS]] · [[VERBO]]
**Tracking:** [[CAMPANHAS-TRACKER]] · [[META-ADS-LOG-DIARIO]]
**Conhecimento:** [[meta-ads-copy-topo]] · [[meta-ads-risco]] · [[META ADS - PLAYBOOK]]
